...
container: $(stage3-config) $(platform-config)  ## Build the container
	...
	http_proxy=http://192.168.0.1:8081 https_proxy=http://192.168.0.1:8081 buildah --storage-driver=vfs --name $(container) from --platform=$(platform) --cap-add=CAP_SYS_PTRACE $(stage3-image)
